  ...
  return (
    <div style={{ display: 'grid', gap: 20 }}>
      ...
      <section style={{ display: 'grid', gap: 12 }}>
        <label>
          <div>Staff Signature Placeholder</div>
          <div style={{ border: '1px dashed #9ca3af', borderRadius: 12, padding: 16, color: '#6b7280' }}>
            Signature pad will be added in a later step.
          </div>
        </label>
        {message ? (
          <p style={{ color: messageType === 'error' ? '#dc2626' : '#2563eb' }}>{message}</p>
        ) : null}
        <button type="button" onClick={handleSubmit} disabled={isPending} style={{ padding: 14, borderRadius: 12, border: 'none', background: '#111827', color: '#fff' }}>
